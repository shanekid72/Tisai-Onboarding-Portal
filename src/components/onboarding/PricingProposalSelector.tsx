...
export const PricingProposalSelector: React.FC<PricingProposalSelectorProps> = ({
  ...
  const handleComplete = () => {
    // Convert our selections to match the PricingSelection interface from partnerOnboarding.ts
    const uniqueRegions = Array.from(new Set(finalSelections.map(sel => sel.region)));
    const selectedCountries = Array.from(new Set(finalSelections.map(sel => sel.country)));
    
    // Group services by country
    const selectedServices: Record<string, string[]> = {};
    finalSelections.forEach(sel => {
      if (!selectedServices[sel.country]) {
        selectedServices[sel.country] = [];
      }
      if (!selectedServices[sel.country].includes(sel.service)) {
        selectedServices[sel.country].push(sel.service);
      }
    });
    ...
    console.log('Completing with pricing selection:', result);
    onComplete(result);
  };

  // Add debug logging
  console.log('PricingProposalSelector render:', { isOpen, loading, error });
  
  // Don't render if not open
  if (!isOpen) {
    console.log('PricingProposalSelector not open, returning null');
    return null;
  }
  
  // Don't use document.querySelector as it may not work during initial render
  // Instead just render with consistent styling that works in both contexts
  
  return (
    <div 
      className="pricing-proposal-overlay" 
      style={{ 
        position: 'relative',
        display: 'flex',
        alignItems: 'center',
    ...
    >
      <div 
        className="pricing-proposal-modal"
        style={{width: '100%', maxWidth: '1000px', backgroundColor: '#fff', borderRadius: '8px'}}
      >
                      ...
                      <div className="pricing-selection-details">
                        ...
                        <div className="pricing-stats">
                          ...
                          <div className="pricing-stat">
                            ...
                            <span className="stat-value">{item.coverage.txnLimit}</span>
                          </div>
                          <div className="pricing-stat">
                        ...
                        >
                          ...
                          <div className="coverage-details">
                            ...
                            <div className="coverage-detail">
                              ...
                              <span className="detail-value">{coverage.txnLimit}</span>
                            </div>
                            <div className="coverage-detail">
